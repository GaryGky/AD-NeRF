...
def get_valid_img_ids():
    ...
    np.save('valid_img_ids.npy', np.array(valid_img_ids))
...
def face_parse():
    ...
    print(f'step3: {face_parsing_cmd}')
...
def decouple_bg():
    ...
    for i in valid_img_ids:
        if os.path.exists(os.path.join(head_imgs_dir, str(i) + '.jpg')) and os.path.exists(
                os.path.join(com_imgs_dir, str(i) + '.jpg')):
            if i % 100 == 0:
                logger.info(f'skip: iter-{i}')
            continue
        ...
        if i % 100 == 0:
            logger.info(f'finish: iter: {i} done')
